...
def reactivation_rate_day(mice, sample_dates):
    ...
    binned_vec = list(np.load(paths['base_path'] + paths['mouse'] + '/data_across_days/y_pred_binned_subset_' + str(sa) + '.npy'))
    ...
    mean_reactivation_mice_original = np.zeros((len(mice), len(binned_vec[0])))
    for mouse in range(0, len(mice)):
        paths = preprocess.create_folders(mice[mouse], sample_dates[mouse])
        binned_vec = list(np.load(paths['base_path'] + paths['mouse'] + '/data_across_days/y_pred_binned_subset_' + str(sa) + '.npy'))
        binned_vec_original = list(np.load(paths['base_path'] + paths['mouse'] + '/data_across_days/y_pred_binned.npy'))
        mean_reactivation = np.zeros((len(binned_vec), len(binned_vec[0])))
        mean_reactivation_original = np.zeros((len(binned_vec), len(binned_vec[0])))
        for i in range(0, len(binned_vec)):
            mean_reactivation[i, :] = binned_vec[i]
            mean_reactivation_original[i, :] = binned_vec_original[i]
        mean = mean_reactivation.mean(axis=0)
        mean_original = mean_reactivation_original.mean(axis=0)
        mean_reactivation_mice[mouse, :] = mean
        mean_reactivation_mice_original[mouse, :] = mean_original

    print(stats.ttest_rel(np.mean(mean_reactivation_mice[:, 1:9], axis=1), np.mean(mean_reactivation_mice_original[:, 1:9], axis=1)))
    ...
    plt.plot(x_label, mean, '-o', c=[.6, 0, .6], linewidth=3, ms=0)
    plt.fill_between(x_label, sem_plus, sem_minus, alpha=0.2, color=[.6, 0, .6], lw=0)
    mean = mean_reactivation_mice_original.mean(axis=0)
    sem_plus = mean + stats.sem(mean_reactivation_mice_original, axis=0)
    sem_minus = mean - stats.sem(mean_reactivation_mice_original, axis=0)
    ...
    sns.despine()
    plt.savefig(paths['base_path'] + '/NN/plots/reactivation_rate_day_subset.png', bbox_inches='tight', dpi=200)
    plt.savefig(paths['base_path'] + '/NN/plots/reactivation_rate_day_subset.pdf', bbox_inches='tight', dpi=200, transparent=True)
    plt.close()
...
def reactivation_difference(mice, sample_dates):
    ...
    for sa in subset_amounts:
        for mouse in range(0, len(mice)):
            paths = preprocess.create_folders(mice[mouse], sample_dates[mouse])
            reactivation_difference_all = np.load(paths['base_path'] + paths['mouse'] + '/data_across_days/reactivation_difference_subset_' + str(sa) + '.npy', allow_pickle=True)
            false_positive = []
            false_negative = []

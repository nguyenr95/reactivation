...
def reactivation_rate(y_pred, behavior):
    ...
    for i in range(0, behavior['dark_runs']+behavior['task_runs']):
        if i == 0:
            ...
            for j in range(0, factor):
                ...
                idx = idx + 1
    return y_pred_binned


def save_reactivation_rate(y_pred_binned, sa, paths, day):
    days_path = paths['base_path'] + paths['mouse'] + '/data_across_days/'
    if path.isfile(days_path + 'y_pred_binned_subset_' + str(sa) + '.npy') == 0 or day == 0:
        np.save(days_path + 'y_pred_binned_subset_' + str(sa), [])
    y_pred_binned_across_days = list(np.load(days_path + 'y_pred_binned_subset_' + str(sa) + '.npy'))
    y_pred_binned_across_days.append(y_pred_binned)
    np.save(days_path + 'y_pred_binned_subset_' + str(sa), y_pred_binned_across_days)
...
def save_reactivation_difference(sum_reactivation_original, false_positive, false_negative, sa, paths, day, days):
    days_path = paths['base_path'] + paths['mouse'] + '/data_across_days/'
    if days:
        if path.isfile(days_path + 'reactivation_difference_subset_' + str(sa) + '.npy') == 0 or day == 0:
            ...
            np.save(days_path + 'reactivation_difference_subset_' + str(sa), reactivation_difference_across_days)
        else:
            reactivation_difference_across_days = np.load(days_path + 'reactivation_difference_subset_' + str(sa) + '.npy',
                                                          allow_pickle=True)
            reactivation_difference_across_days[0][day] = sum_reactivation_original
            reactivation_difference_across_days[1][day] = false_positive
            reactivation_difference_across_days[2][day] = false_negative
